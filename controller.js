...
export async function getData(req, res, next) {
    ...
    let order = req.query.order ? String(req.query.order).toLowerCase() : "asc"
    ...
    const data = DB.getData(req.params.table, limit, offset, order)
    let result = []
    if (data) {
...
export async function insertData(req, res, next) {
    ...
    // });
    const indianTime = new Date().toISOString()
    const data = DB.insertData(req.params.table, JSON.stringify(req.body), indianTime)
